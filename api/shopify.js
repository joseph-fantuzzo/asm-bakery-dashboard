...
async function shopifyGQL(token, query, variables) {
  ...
  if (json.errors && json.errors.length) throw new Error('GraphQL: ' + json.errors.map(e => e.message).join('; '));
  return json.data;
}
...
module.exports = async function handler(req, res) {
  ...
  if (!token) return res.status(401).json({ error: 'Shopify not connected — run OAuth first' });
  ...
  try {
    ...
    if (!action || action === 'getOrders') {
      const data = await shopifyGQL(token, `{
        orders(first: 100, query: "fulfillment_status:unfulfilled", sortKey: CREATED_AT, reverse: true) {
          edges { node {
            id name displayFulfillmentStatus createdAt
            totalPriceSet { shopMoney { amount currencyCode } }
            customer { firstName lastName email }
            lineItems(first: 50) { edges { node { id name quantity sku } } }
            note
          }}
        }
      }`);
    ...
    if (action === 'fulfillOrder') {
      ...
      return res.status(200).json({ success: true, fulfillment: fData?.fulfillmentCreateV2?.fulfillment });
    }
